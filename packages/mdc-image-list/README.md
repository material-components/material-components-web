...
`mdc-image-list-shape-radius($radius, $rtl-reflexive)` | Sets the rounded shape of image list item with given radius size. Set `$rtl-reflexive` to true to flip radius values in RTL context, defaults to false.
