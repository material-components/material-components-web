...
| `onDown(event) => void` | Starts interaction with the slider when modifying the value |
| `onMove(event) => void` | Continues interaction with the slider, modifying the value based on the event `pageX` and triggering `notifyInput()` |
| `onEnd() => void` | Ends interaction with the slider when modifying the value, triggering `notifyChange()` |
