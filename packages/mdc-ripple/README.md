...
> _NOTE_: The `mdc-states-activated` and `mdc-states-selected` mixins add appropriate styles to elements with `{root-element}--activated` or `{root-element}--selected` modifier classes.
