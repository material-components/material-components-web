...
`mdc-card-shape-radius($radius, $rtl-reflexive)` | Sets the rounded shape of given radius size for card. Set `$rtl-reflexive` to true to flip radius values in RTL context, defaults to false.
