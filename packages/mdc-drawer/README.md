...
`mdc-drawer-shape-radius($radius, $rtl-reflexive)` | Sets the rounded shape for drawer with given radius size. Set `$rtl-reflexive` to true to flip radius values in RTL context, defaults to true.
`mdc-drawer-item-shape-radius($radius, $rtl-reflexive)` | Sets the rounded shape for drawer navigation item with given radius size. Set `$rtl-reflexive` to true to flip radius values in RTL context, defaults to true.
