...
`mdc-drawer-shape-radius($radius, $rtl-reflexive)` | Sets the rounded shape to drawer with given radius size. `$radius` can be single radius or list of 2 radius values for trailing-top and trailing-bottom. Set `$rtl-reflexive` to true to flip radius values in RTL context, defaults to true.
