...
export default class MDCPersistentDrawerFoundation extends MDCFoundation {
  ...
  constructor(adapter) {
    super(Object.assign(MDCPersistentDrawerFoundation.defaultAdapter, adapter));

    this.transitionEndHandler_ = (ev) => {
      if (this.adapter_.isDrawer(ev.target)) {
        this.adapter_.removeClass(MDCPersistentDrawerFoundation.cssClasses.ANIMATING);
  ...
  init() {
    const {ROOT, OPEN} = MDCPersistentDrawerFoundation.cssClasses;
  ...
  open() {
    ...
    this.adapter_.addClass(MDCPersistentDrawerFoundation.cssClasses.ANIMATING);
    this.adapter_.addClass(MDCPersistentDrawerFoundation.cssClasses.OPEN);
    this.retabinate_();
    this.isOpen_ = true;
  }
  ...
  close() {
    ...
    this.adapter_.registerTransitionEndHandler(this.transitionEndHandler_);
    this.adapter_.addClass(MDCPersistentDrawerFoundation.cssClasses.ANIMATING);
    this.adapter_.removeClass(MDCPersistentDrawerFoundation.cssClasses.OPEN);
    this.detabinate_();
    this.isOpen_ = false;
  }
  ...
  handleTouchStart_(evt) {
    if (!this.adapter_.hasClass(MDCPersistentDrawerFoundation.cssClasses.OPEN)) {
      return;
    }
