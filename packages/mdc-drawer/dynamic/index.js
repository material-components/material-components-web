...
import MDCDynamicDrawerFoundation from './foundation';
import {states} from './constants';
import * as util from '../util';

export {MDCDynamicDrawerFoundation};
export {util};

export class MDCDynamicDrawer extends MDCComponent {
  static attachTo(root) {
    return new MDCDynamicDrawer(root);
  }
  ...
  set state(value) {
    if (this.state_ !== value) {
      if (states.includes(value)) {
        ...
        this.state_ = value;
        this.root_.classList.add('mdc-dynamic-drawer__' + value);
      } else {
        throw new Error('"' + value + '" is not allowed state. Allowed states are ' + states.join(', '));
      }
    }
  }
  ...
  get drawer() {
    return this.root_.querySelector(MDCDynamicDrawerFoundation.strings.DRAWER_SELECTOR);
  }

  getDefaultFoundation() {
    const {FOCUSABLE_ELEMENTS, OPACITY_VAR_NAME} = MDCDynamicDrawerFoundation.strings;

    return new MDCDynamicDrawerFoundation({
      ...
      notifyOpen: () => this.emit(MDCDynamicDrawerFoundation.strings.OPEN_EVENT),
      notifyClose: () => this.emit(MDCDynamicDrawerFoundation.strings.CLOSE_EVENT),
