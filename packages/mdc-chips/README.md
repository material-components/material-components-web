...
--- | --- | ---
foundation_ | `MDCChipFoundation` | The foundation class for this component. This is either passed in as an optional second argument to the constructor, or assigned the result of calling `getDefaultFoundation()`
id | `string` | Unique identifier on the chip. \*_NOTE_: This will be the same as the `id` attribute on the root element. If an `id` is not provided, a unique one will be generated by `MDCChipSet.addChip()`.
selected | `boolean` | Sets selected state on the chip. Proxies to the foundation's `isSelected`/`setSelected` methods
shouldRemoveOnTrailingIconClick | `boolean` | Sets whether a trailing icon click should trigger exit/removal of the chip. Proxies to the foundation's `getShouldRemoveOnTrailingIconClick`/`setShouldRemoveOnTrailingIconClick` methods. \*_NOTE_: If `shouldRemoveOnTrailingIconClick` is set to false, you must manually call `beginExit()` on the chip to remove it
...
Name | Type | Description
--- | --- | ---
foundation_ | `MDCChipSetFoundation` | The foundation class for this component. This is either passed in as an optional second argument to the constructor, or assigned the result of calling `getDefaultFoundation()`
root_ | `Element` | The root element passed into the constructor as the first argument.
selectedChipIds | `ReadonlyArray<string>` | @returns An array of the IDs of all selected chips.
...
Signature | Description
--- | ---
`notifyInteraction() => void` | Emits a custom "MDCChip:interaction" event denoting the chip has been interacted with (typically on click or keydown). Notifies the Chip Set that the chip has been interacted with. \*_NOTE_: `notifyInteraction` and `notifyTrailingIconInteraction` must pass along the target chip's ID, and must be observable by the parent `mdc-chip-set` element (e.g. via DOM event bubbling).
`addClass(className: string) => void` | Adds a class to the root element.
`eventTargetHasClass(target: EventTarget | null, className: string) => boolean` | Returns true if target has className, false otherwise.
`getCheckmarkBoundingClientRect() => ClientRect | null` | Returns the bounding client rect of the root element.
...
`notifyRemoval() => void` | Emits a custom event "MDCChip:removal" denoting the chip will be removed. \*_NOTE_: `notifyRemoval` must pass along the target chip's ID and its root element, and must be observable by the parent `mdc-chip-set` element (e.g. via DOM event bubbling).
`notifySelection(selected: boolean) => void` | Emits a custom "MDCChip:selection" event denoting the chip has been selected or deselected. Notifies the Chip Set that the chip has been selected or deselected. \*_NOTE_: `notifySelection` must pass along the target chip's ID and selected state, and must be observable by the parent `mdc-chip-set` element (e.g. via DOM event bubbling).
`notifyTrailingIconInteraction() => void` | Emits a custom "MDCChip:trailingIconInteraction" event denoting the trailing icon has been interacted with (typically on click or keydown). Notifies the Chip Set that the chip's trailing icon has been interacted with. \*_NOTE_: `notifyInteraction` and `notifyTrailingIconInteraction` must pass along the target chip's ID, and must be observable by the parent `mdc-chip-set` element (e.g. via DOM event bubbling).
...
Signature | Description
--- | ---
`handleTrailingIconInteraction(evt: MouseEvent | KeyboardEvent) => void` | Handles an interaction event on the trailing icon element. This is used to prevent the ripple from activating on interaction with the trailing icon.
`beginExit() => void` | Begins the exit animation which leads to removal of the chip.
...
`handleInteraction(evt: MouseEvent | KeyboardEvent) => void` | Handles an interaction event on the root element.
...
`handleChipSelection(chipId: string, selected: boolean) => void` | Handles a chip selection event, used to handle discrepancy when selection state is set directly on the Chip.
`select(chipId: string) => void` | Selects the chip with the given id. Deselects all other chips if the chip set is of the choice variant.
