...
Input chips are removed in response to clicking the trailing remove icon in the chip if the trailing <i> element has the class `mdc-chip-trailing-action`. Removal can also be triggered by calling `MDCChip`'s `beginExit()` method.
