...
import MDCDynamicDrawerFoundation from '../../../packages/mdc-drawer/dynamic/foundation';
import {cssClasses, strings} from '../../../packages/mdc-drawer/dynamic/constants';

function setupTest() {
  const {foundation, mockAdapter} = setupFoundationTest(MDCDynamicDrawerFoundation);
  td.when(mockAdapter.hasClass('mdc-dynamic-drawer')).thenReturn(true);
  td.when(mockAdapter.hasNecessaryDom()).thenReturn(true);
  return {foundation, mockAdapter};
}

suite('MDCDynamicDrawerFoundation');

test('exports strings', () => {
  assert.deepEqual(MDCDynamicDrawerFoundation.strings, strings);
});

test('exports cssClasses', () => {
  assert.deepEqual(MDCDynamicDrawerFoundation.cssClasses, cssClasses);
});

test('defaultAdapter returns a complete adapter implementation', () => {
  verifyDefaultAdapter(MDCDynamicDrawerFoundation, [
...
test('on touch start updates the drawer to the touch target coordinates', () => {
  ...
  td.when(mockAdapter.hasClass('mdc-dynamic-drawer--open')).thenReturn(true);
...
test('on touch start does not update the drawer when drawer not open', () => {
  ...
  const handlers = captureHandlers(mockAdapter, 'registerDrawerInteractionHandler');
  const raf = createMockRaf();
  td.when(mockAdapter.hasClass('mdc-dynamic-drawer--open')).thenReturn(false);
  td.when(mockAdapter.getDrawerWidth()).thenReturn(500);
  foundation.init();
