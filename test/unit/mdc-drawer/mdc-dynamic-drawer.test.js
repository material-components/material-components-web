...
import {MDCDynamicDrawer} from '../../../packages/mdc-drawer/dynamic';
import {strings} from '../../../packages/mdc-drawer/dynamic/constants';
import {getTransformPropertyName, supportsCssCustomProperties} from '../../../packages/mdc-drawer/util';

function getFixture() {
  return bel`
    <aside class="mdc-dynamic-drawer">
...
function getTemporaryFixture() {
  return bel`
    <aside class="mdc-dynamic-drawer mdc-dynamic-drawer__temporary">
      <nav class="mdc-dynamic-drawer__drawer">
      </nav>
    </aside>
  `;
...
function setupTest() {
  ...
  const component = new MDCDynamicDrawer(root);
  return {root, component};
}

suite('MDCDynamicDrawer');

test('attachTo initializes and returns a MDCDynamicDrawer instance', () => {
  assert.isOk(MDCDynamicDrawer.attachTo(getFixture()) instanceof MDCDynamicDrawer);
});
...
test('get/set open', () => {
  ...
  component.open = true;
  assert.isOk(root.classList.contains('mdc-dynamic-drawer--open'));
  assert.isOk(component.open);
  td.verify(openHandler(td.matchers.anything()));

  component.open = false;
  assert.isNotOk(root.classList.contains('mdc-dynamic-drawer--open'));
  assert.isNotOk(component.open);
  td.verify(closeHandler(td.matchers.anything()));
});

test('proper state initialization', () => {
  const persistentRoot = getPersistentFixture();
  const persistentComponent = new MDCDynamicDrawer(persistentRoot);
  assert.equal(persistentComponent.state, 'persistent');

  const temporaryRoot = getTemporaryFixture();
  const temporaryComponent = new MDCDynamicDrawer(temporaryRoot);
  assert.equal(temporaryComponent.state, 'temporary');
...
test('get/set state', () => {
  ...
  assert.isNotOk(root.classList.contains('mdc-dynamic-drawer__temporary'));
  assert.isNotOk(root.classList.contains('mdc-dynamic-drawer__persistent'));

  component.state = 'temporary';
  assert.isOk(root.classList.contains('mdc-dynamic-drawer__temporary'));
  assert.equal(component.state, 'temporary');

  component.state = 'persistent';
  assert.isNotOk(root.classList.contains('mdc-dynamic-drawer__temporary'));
  assert.isOk(root.classList.contains('mdc-dynamic-drawer__persistent'));
  assert.equal(component.state, 'persistent');
});

test('throws an error when seted not existing state', () => {
  const {component} = setupTest();

  assert.throws(() => component.state = 'not-existing-state');
});
...
test('adapter#makeElementUntabbable sets a tab index of -1 on the element', () => {
  const root = bel`
    <aside class="mdc-dynamic-drawer mdc-dynamic-drawer--open">
      <nav class="mdc-dynamic-drawer__drawer">
        <a id="foo" href="foo"></a>
      </nav>
    </aside>
...
test('adapter#isRtl returns true for RTL documents', () => {
  const root = bel`
    <aside dir="rtl" class="mdc-dynamic-drawer">
      <nav class="mdc-dynamic-drawer__drawer">
      </nav>
    </aside>
  `;
...
test('adapter#isRtl returns false for explicit LTR documents', () => {
  const root = bel`
    <aside dir="ltr" class="mdc-dynamic-drawer">
      <nav class="mdc-dynamic-drawer__drawer">
      </nav>
    </aside>
  `;
