...
import MDCTabScrollingFoundation from '../../../packages/mdc-tab-scroller/scrolling-foundation';

suite('MDCTabScrollingFoundation');

const setupTest = () => setupFoundationTest(MDCTabScrollingFoundation);
...
test(`#handleInteraction() removes the ${MDCTabScrollingFoundation.cssClasses.ANIMATING} class`, () => {
  const {foundation, mockAdapter} = setupHandleInteractionTest();
  foundation.handleInteraction();
  td.verify(mockAdapter.removeClass(MDCTabScrollingFoundation.cssClasses.ANIMATING), {times: 1});
});
...
test(`#handleTransitionEnd() removes the ${MDCTabScrollingFoundation.cssClasses.ANIMATING} class`, () => {
  const {foundation, mockAdapter} = setupTest();
  foundation.handleTransitionEnd();
  td.verify(mockAdapter.removeClass(MDCTabScrollingFoundation.cssClasses.ANIMATING));
});
...
test(`#scrollTo() adds the ${MDCTabScrollingFoundation.cssClasses.ANIMATING} class in a rAF`, () => {
  const {foundation, mockAdapter} = setupScrollToTest();
  const raf = createMockRaf();
  foundation.scrollTo(100);
  raf.flush();
  raf.restore();
  td.verify(mockAdapter.addClass(MDCTabScrollingFoundation.cssClasses.ANIMATING), {times: 1});
});
...
test(`#scrollTo() removes the ${MDCTabScrollingFoundation.cssClasses.ANIMATING} if called during an animation`,
  () => {
    const {foundation, mockAdapter} = setupScrollToTest({
      scrollLeft: 50,
      rootWidth: 100,
...
test(`#incrementScroll() removes the ${MDCTabScrollingFoundation.cssClasses.ANIMATING} if called during an animation`,
  () => {
    const {foundation, mockAdapter} = setupScrollToTest({
      scrollLeft: 50,
      rootWidth: 100,
