...
suite('MDCTabScrollerFoundation - RTL Negative Scrolling');

test(`#computeRTLScrollDirection_() returns ${MDCTabScrollerFoundation.strings.RTL_NEGATIVE}`, () => {
  ...
  assert.strictEqual(foundation.computeRTLScrollDirection_(), MDCTabScrollerFoundation.strings.RTL_NEGATIVE);
});

const setupNegativeTest = ({scrollLeft, contentWidth, rootWidth, transform='none'}) => {
  const {foundation, mockAdapter} = setupFoundationTest(MDCTabScrollerFoundation);
  ...
  foundation.rtlScrollType_ = MDCTabScrollerFoundation.strings.RTL_NEGATIVE;
  return {foundation, mockAdapter};
};


test('#computeCurrentScrollPosition() returns the delta of the max and current scroll values', () => {
  const {foundation} = setupNegativeTest({
    scrollLeft: -123,
    contentWidth: 666,
    rootWidth: 200,
  });
  assert.strictEqual(foundation.computeCurrentScrollPosition(), -123);
});

test('#scrollTo() sets the scrollLeft property to scroll value', () => {
  const {foundation, mockAdapter} = setupNegativeTest({
    scrollLeft: -100,
    contentWidth: 2000,
    rootWidth: 360,
  });
