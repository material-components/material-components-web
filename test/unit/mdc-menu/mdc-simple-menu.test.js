...
import {strings, events} from '../../../packages/mdc-menu/simple/constants';
...
test(`adapter#notifySelected fires an ${events.SELECTED_EVENT} custom event with the item and index`, () => {
  const {root, component} = setupTest();
  ...
  root.addEventListener(events.SELECTED_EVENT, handler);
...
test(`adapter#notifyCancel fires an ${events.CANCEL_EVENT} custom event`, () => {
  const {root, component} = setupTest();
  const handler = td.func('notifyCancel handler');
  root.addEventListener(events.CANCEL_EVENT, handler);
