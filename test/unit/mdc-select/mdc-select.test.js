...
import {events} from '../../../packages/mdc-select/constants';
...
test(`adapter#notifyChange emits an ${events.CHANGE_EVENT} custom event from the root element`, () => {
  const {component, fixture} = setupTest();
  const handler = td.func('change handler');
  fixture.addEventListener(events.CHANGE_EVENT, handler);
