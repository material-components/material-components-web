...
test('on page up keydown increases the slider by 5 when no step given', () => {
  const {foundation, mockAdapter, raf, rootHandlers} = setupTest();
  ...
  td.verify(mockAdapter.setThumbStyleProperty(TRANSFORM_PROP, 'translateX(45px) translateX(-50%)'));
  td.verify(mockAdapter.setTrackFillStyleProperty(TRANSFORM_PROP, 'scaleX(45)'));

  raf.restore();
});

test('on page up keydown increases the slider by 5 * the step value when a step is given', () => {
  const {foundation, mockAdapter, raf, rootHandlers} = setupTest();
...
test('on page down keydown increases the slider by 5 when no step given', () => {
  const {foundation, mockAdapter, raf, rootHandlers} = setupTest();
...
test('on page down keydown increases the slider by 5 * the step value when a step is given', () => {
  const {foundation, mockAdapter, raf, rootHandlers} = setupTest();
  ...
  assert.equal(foundation.getValue(), 25);
  td.verify(mockAdapter.setThumbStyleProperty(TRANSFORM_PROP, 'translateX(25px) translateX(-50%)'));
  td.verify(mockAdapter.setTrackFillStyleProperty(TRANSFORM_PROP, 'scaleX(25)'));
